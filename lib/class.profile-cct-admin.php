...
class Profile_CCT_Admin {
	 ...
	 * @return void
	 */
	public static function update_clone_fields(){
		global $blog_id;
	...
	static function get_option( $type = 'form', $fields_or_tabs = 'fields', $context = 'normal' ){
		...
		if ( isset( self::$option[ $type ][ $fields_or_tabs ][ $context ] ) && is_array( self::$option[ $type ][ $fields_or_tabs ][ $context ] ) ):
			...
			if ( ! is_array( $options ) ) :
				...
				if ( 'fields' === $fields_or_tabs ) :
					$options = ( isset( $default[ $fields_or_tabs ][ $context ] ) ) ? $default[ $fields_or_tabs ][ $context ] : array();
				else :
					$options = isset( $default[ $fields_or_tabs ] ) ? $default[ $fields_or_tabs ] : array();
				endif;
			endif;
				...
				if ( is_array( $fields ) ):
					foreach ( $fields as $field ):
						...
						if ( function_exists( 'profile_cct_' . $field['type'] . '_shell' ) ) :
							...
							do_action( 'profile_cct_shell_' . $field['type'], $field, $field_data );
						endif;
					endforeach;
				endif;
				echo $end_shell;
	 ...
	 * @return void
	 */
	public static function insert_post( $post_id, $post_data ) {
