...
class Profile_CCT {
	...
	function sort_posts( $query ) {
		...
		if ( $query->is_main_query() && ( $is_taxonomy || is_post_type_archive( 'profile_cct' ) || ( isset( $query->query_vars['post_type'] ) && $query->query_vars['post_type'] == 'profile_cct' ) && isset( $query->query_vars['s'] ) && $query->query_vars['s'] != '' ) && ! is_admin() ):
			...
			switch ( $this->settings['sort_order_by'] ):
				...
				case "last_name":
					...
					// 	'key' => 'profile_cct_last_name',
					// 	'value' => $searchTerm,
					// 	'compare' => 'LIKE'
					// ) );
					// $query->set( 'meta_query', $meta_query );
					$query->set( 'post_type', 'profile_cct' );
					if ( is_null( $order ) ) $order = 'ASC';
					break;
				case "date":
