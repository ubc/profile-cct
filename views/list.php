<?php
	if ( isset( $_REQUEST['reset'] ) && isset( $_REQUEST['nonce'] ) && wp_verify_nonce( $_REQUEST['nonce'], 'profile_cct_reset_fields' ) ):
		...
		foreach( self::get_contexts() as $context ):
			Profile_CCT_Admin::delete_option( 'list', 'fields', $context, array() );
		endforeach;
		
		?>
		<script> window.location = "<?php echo admin_url( 'edit.php?post_type=profile_cct&page='.PROFILE_CCT_BASEADMIN.'&view=list' ); ?>"; </script>
		<?php
	endif;
	
	$reset_url = admin_url( 'edit.php?post_type=profile_cct&page='.PROFILE_CCT_BASEADMIN.'&view=list&reset=1&nonce='.wp_create_nonce( 'profile_cct_reset_fields' ) );
?>
