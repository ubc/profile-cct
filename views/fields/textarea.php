...
function profile_cct_textarea_field( $data, $options ){
	...
	$field->input_field( array( 
		...
		'row'=>2, 
		'cols'=>20, 
		'value'=> $data['textarea'], 'type'=>'textarea' 
	));
...
function profile_cct_textarea_display_shell( $action, $options, $data=null ) {
	...
	if( !empty( $data['textarea'] ) || !empty( $options['empty'] ) || $action == "edit" ):
		$field->start_field($action,$options);
...
function profile_cct_textarea_display( $data, $options ){
	...
	$text_value = (!empty($data['textarea']) ? $data['textarea'] : $empty );
		
	$field->display_text( array( 
		'field_type'=>$type, 
		'class'=>'textarea textarea',
		'default_text'=>'lorem ipsum', 
		'content_filter'=>'profile_escape_html', 
		'value'=>$text_value, 
		'tag'=>'div', 
		'type'=>'text' ) );
}
